...
scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, milestones=list(range(3,n_epochs)), gamma=0.8)
...
lines = ['\nepoch,avg_train_loss,avg_train_mlm_acc,avg_train_nsp_acc,avg_val_loss,avg_val_mlm_acc,avg_val_nsp_acc']
...
for i_epoch in range(n_epochs):
    ...
    n_preds = 0
    nsp_acc_sum = 0
    for ib, batch in enumerate(tqdm(dl_train, desc='Train, Epoch #{}'.format(i_epoch))):
    ...
    avg_train_mlm_acc = mlm_acc_sum / n_preds
    avg_train_nsp_acc = nsp_acc_sum / n_items
    avg_train_loss = loss_sum / n_items
    ...
    avg_val_mlm_acc = mlm_acc_sum / n_preds
    avg_val_nsp_acc = nsp_acc_sum / n_items
    avg_val_loss = loss_sum / n_items
    
    tmp = [i_epoch, avg_train_loss, avg_train_mlm_acc, avg_train_nsp_acc, avg_val_loss, avg_val_mlm_acc, avg_val_nsp_acc]
